import { ObjectPaths, ObjectValueByPath } from '@properly-typed/utils';
...
export const typedGet = <
  ...
  P extends ObjectPaths<T, D>,
  D extends string = '/',
>(target: T, path: P, delimiter?: D): ObjectValueByPath<T, P, D> => {
  if (!path) {
    return target as ObjectValueByPath<T, P, D>;
  }
